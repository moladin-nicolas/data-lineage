...
if uploaded_file is not None:
    ...
    with col1:
        # create select box
        selected_table = st.selectbox('Select a table', df_nodes.table.to_list())
        # filter data based on select box input
        df_edges_ = get_lineage(df_edges, selected_table)
        df_nodes_ = get_nodes(df_edges_)

        # button to refresh lineage
        if st.button('Show lineage') and selected_table is not None:
            ...
            if flow_key in st.session_state and flow_key:
                ...
                st.session_state['nodes'] = [
                    StreamlitFlowNode(
                        ...
                        style={'word-wrap': 'break-word', 'backgroundColor': 'green'} if row.table == 'x' else {'word-wrap': 'break-word', 'width': 200}
